...
from app.ui.theme import inject_polish, render_steps_card, render_hero_banner
...
from app.features.resume.agent import RESUME_AGENT_MODE, render_resume_agent
...
# 🔐 Auth System
# -------------------------------
# USERS_FILE removed — user storage is Supabase (app/core/db.py), not a local JSON file.
_ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
QUESTION_BANK_FILE = os.path.join(_ROOT, "data", "question_bank.json")
...
if not st.session_state["logged_in"]:
    ...
    if _demo == "jobs":
        ...
        try:
            from app.demos import job_search as demo_job_search
    ...
    if _demo == "interview":
        ...
        if _b2.button("← Back to home", use_container_width=True, key="demo_iv_back"):
            _clear_demo(); st.rerun()
        try:
            from app.demos import mock_interview as demo_mock_interview
    ...
    if st.session_state.get("auth_page", "login") == "login":
        from app.ui.auth_views import render_login_page
    ...
    if st.session_state.get("auth_page") == "signup":
        from app.ui.auth_views import render_signup_page
    ...
    if st.session_state.get("auth_page") == "forgot":
        from app.ui.auth_views import render_forgot_page
        render_forgot_page(
    ...
    _nit_img_tag = ""
    _nit_path = os.path.join(_ROOT, "assets", "images", "Nit.png")
    ...
    _robot_img_tag = ""
    _robot_path = os.path.join(_ROOT, "assets", "images", "Robot.png")
...
""",
    ...
    def _enc(fname):
        ...
        p = os.path.join(_ROOT, "assets", "images", fname)
        ...
        if total_q == 0:
            ...
            with colA:
                if st.button("📧 Email me this report card", use_container_width=True):
                    try:
                        from app.core import email as report_email
                        user_email = st.session_state.get("user_email", "")
...
if language_mode == JOB_SEARCH_MODE:
    ...
    from app.features.jobs import agent as job_search_agent
